...
for(j in 1:100){
  for(i in unique(metadata$LUBand)){
    ...
    zeta_N <- zetaDecay$zeta.val[5] #Higher order zeta diversity measure.
    zeta_Nsd <- zetaDecay$zeta.val.sd[5] #Higher order zeta diversity measure standard deviation.
...
CSCImodel <- train(meanCSCI~zeta_1+zeta_2+zeta_3+zeta_4+zeta_N,data=zetaAnalysis,method="lm",trControl=train.control)
print(CSCImodel)
