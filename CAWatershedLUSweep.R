...
GISBioData$Watershed <- gsub(" ","",GISBioData$Watershed,fixed=TRUE)
...
for(WS in watersheds$Watershed){
  ...
  for(i in 1:2){
    if(i==1){
      ...
      meanLU = mean(na.omit(GISBioDataSubsetLow$LU_2000_5K))
      }
    if(i==2){
      lowNum = as.integer((sampleNum-groupNum)+1)
      highNum = as.integer(sampleNum)
      selected <- GISBioDataSubsetHigh
      meanLU = mean(na.omit(GISBioDataSubsetHigh$LU_2000_5K))
      }
    ...
    filename = paste("LUSweepCAWatershed",gsub(" ","",WS,fixed=TRUE),groupNum,"Samples",lowNum,"to",highNum,"S",spotNum,"R",repNum,"M",meanLU,sep="")
...
networkfiles <- Sys.glob("AgSweepCAWatershed*50Samples*Network.txt")
...
for(networkFile in networkfiles){
  ...
  meanLU <- as.numeric(str_match(networkFile, "(.*?)SweepCAWatershed(.*?)50Samples(.*?)M(.*?)Network")[5])
  ...
  dat[1,40] <- Watershed <- str_match(networkFile,"AgSweepCAWatershed(.*?)50Samples")[2]
...
networkAnalysis <- read.table("LU_2000_5KWatershed30SamplesSweepCA.txt",header=TRUE, sep="\t",as.is=T,skip=0,fill=TRUE,check.names=FALSE)
networkAnalysis <- join(networkAnalysis,gammaDiversity,by=c("filename"))
#networkAnalysis <- join(networkAnalysis,SCCWRP[,c("Watershed","CatalogingUnitAreaSqMi")])
#networkAnalysis <- networkAnalysis[!duplicated(networkAnalysis),]
#write.table(networkAnalysis,"LU_2000_5KWatershed50SamplesSweepCA.txt",quote=FALSE,sep="\t",row.names = FALSE)
