...
#par(mar=c(0,0,0,0))
hist(degree(networkgraphCon),xlim=range(1:40),xlab="Node degree",ylim=range(1:20),main="",breaks=40)
#png(filename=paste(networkFile,"Nodes",networkNodecount,"Edges",networkEdgecount,"Con.png",sep=""))
#plot(networkgraphCon,vertex.label=NA,vertex.size=degree(networkgraphCon,mode="total",loops=FALSE),edge.width=-1*E(networkgraphCon)$weight, edge.color="red", normalized=TRUE,layout=layout.lgl(networkgraphCon))
dev.off()
...
#par(mar=c(0,0,0,0))
#plot(networkgraphCov,vertex.label=NA,vertex.size=degree(networkgraphCov,mode="total",loops=FALSE),edge.width=1*E(networkgraphCov)$weight, edge.color="blue", normalized=TRUE,layout=layout.lgl(networkgraphCov))
hist(degree(networkgraphCov),xlim=range(1:40),xlab="Node degree",ylim=range(1:20),main="",breaks=40)
dev.off()
...
dev.off()
plot(networkAnalysis$meanLU, networkAnalysis$l_con_rM, type='p', xlim=c(0.0,100.0), ylim=c(-2.5,0.5), xlab='Land Use', ylab='Log-ratio modularity',main="Log-ratio modularity and land use",col="red")
par(new=T)
plot(networkAnalysis$meanLU, networkAnalysis$l_cov_rM, type='p', xlim=c(0.0,100.0), ylim=c(-2.5,0.5), xlab='Land Use', ylab='Log-ratio modularity',col="blue")
par(new=F)
dev.off()
plot(networkAnalysis$meanLU, networkAnalysis$zeta_Con, type='p', xlim=c(0.0,100.0), ylim=c(1.0,2.5), xlab='Land Use', ylab='Degree Heterogeneity',main="Degree Heterogeneity and land use",col="red")
par(new=T)
plot(networkAnalysis$meanLU, networkAnalysis$zeta_Cov, type='p', xlim=c(0.0,100.0), ylim=c(1.0,2.5), xlab='Land Use', ylab='Degree Heterogeneity',col="blue")
par(new=F)
dev.off()
plot(networkAnalysis$meanLU, networkAnalysis$con_C, type='p', xlim=c(0.0,100.0), ylim=c(0.05,0.2), xlab='Land Use', ylab='Connectance',main="Connectance and land use",col="red")
par(new=T)
plot(networkAnalysis$meanLU, networkAnalysis$cov_C, type='p', xlim=c(0.0,100.0), ylim=c(0.05,0.2), xlab='Land Use', ylab='Connectance',col="blue")
par(new=F)
dev.off()
plot(networkAnalysis$meanLU, networkAnalysis$meanStrength_Con, type='p', xlim=c(0.0,100.0), ylim=c(0.0,0.045), xlab='Land Use', ylab='Mean Correlation Strength',main="Mean Correlation Strength and land use",col="red")
par(new=T)
plot(networkAnalysis$meanLU, networkAnalysis$meanStrength_Cov, type='p', xlim=c(0.0,100.0), ylim=c(0.0,0.045), xlab='Land Use', ylab='Mean Correlation Strength',col="blue")
par(new=F)
dev.off()
plot(networkAnalysis$meanLU, networkAnalysis$lambda_network_m_Con, type='p', xlim=c(0.0,100.0), ylim=c(0.7,6.5), xlab='Land Use', ylab='Resilience',main="Resilience and land use",col="red")
par(new=T)
plot(networkAnalysis$meanLU, networkAnalysis$lambda_network_m_Cov, type='p', xlim=c(0.0,100.0), ylim=c(0.7,6.5), xlab='Land Use', ylab='Resilience',col="blue")
par(new=F)
...
dev.off()
MapCoordinates <- data.frame(SCCWRP$Simpson,SCCWRP$Longitude,SCCWRP$Latitude)
colnames(MapCoordinates) = c('Simpson','lon','lat')
...
CalMap <- ggmap(CalMap)+geom_point(data = MapCoordinates, mapping = aes(x = lon, y = lat, color = Simpson))+scale_colour_gradient(low = "green", high = "red", space = "Lab", na.value = "grey50", guide = "colourbar")+ theme(legend.position="top")
