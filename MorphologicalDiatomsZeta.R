...
#Filter algal reads to only keep diatoms.
uniqueAlgae <- uniqueAlgae[uniqueAlgae$class=="Bacillariophyceae",]
...
ASCI <- ASCI[ASCI$assemblage=="Diatom" & ASCI$metric=="ASCI" & ASCI$replicate==1,]
...
write.table(zetaAnalysis,paste("zetaAnalysisMorphologicalDiatoms",taxonomicLevel,".txt",sep=""),quote=FALSE,sep="\t",row.names = FALSE)
#

##To run locally.
taxonomicLevel <- "order"
zetaAnalysis <- read.table(paste("zetaAnalysisMorphologicalDiatoms",taxonomicLevel,".txt",sep=""), header=TRUE, sep="\t",as.is=T,skip=0,fill=TRUE,check.names=FALSE, encoding = "UTF-8")
